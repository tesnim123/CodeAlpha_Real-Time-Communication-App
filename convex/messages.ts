...
export const get = query({
  ...
  handler: async (ctx, args) => {
    ...
    const membership=await ctx.db.query("conversationMembers").withIndex("by_memberId_conversationId",
    ...
    withIndex("by_conversationId",q=>q.eq("conversationId",args.id)).order("desc").collect();
