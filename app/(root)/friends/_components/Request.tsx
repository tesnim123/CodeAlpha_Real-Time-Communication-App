...
const Request = ({id,imageUrl,username,email}: Props) => {
  ...
  return <Card className="w-full p-2 flex flex-row items-center justify-between gap-2">
    ...
    <div className='flex items-center gap-2'>
        <Button size="icon" disabled={denyPending||acceptPending} onClick={()=>{acceptRequest({id}).then(()=>{toast.success("Friend request accepted")}).catch((error)=>{
        ...
        <Button size="icon" disabled={denyPending||acceptPending} variant={'destructive'} onClick={()=>{
          denyRequest({id}).then(()=>{toast.success("Friend request denied")}).catch((error)=>{
            toast.error(error instanceof ConvexError ? error.data :"Unexpected error occured")
          })
